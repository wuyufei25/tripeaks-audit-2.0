...
def audit_engine(row, col_map, base_init_score, burst_window, burst_threshold):
    ...
    return score, ",".join(red_tags) if red_tags else "通过", c1, c2, c3, relay, f1, f2, " | ".join(
        breakdown), max_combo, long_combo_cnt, valid_hand_cnt
    ...
    if raw_list:
        ...
        with st.spinner('执行红线并集概率审计...'):
            audit_res = main_df.apply(lambda r: pd.Series(audit_engine(r, cm, base_score, burst_win, burst_thr)),
                                      axis=1)
            main_df[['得分', '红线判定', 'c1', 'c2', 'c3', '接力', 'f1', 'f2', '得分构成', '最长连击', '长连次数',
                     '有效手牌']] = audit_res
        ...
        with st.sidebar:
            ...
            for k, v in export_cols.items():
                if k is not None and k in export_df.columns:
                    ...
                    if k is None:
                        export_df[v] = 'N/A'  # 仅当原列名都没找到时才补N/A
                    else:
                        final_export_cols[k] = v  # 找到了原列名但可能有其他问题，照常映射
